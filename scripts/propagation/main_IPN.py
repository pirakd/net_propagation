...
from args import MockArgs, DeltaArgs, CovJanArgs
import numpy as np
from tqdm import tqdm
from random import shuffle

test_name = path.basename(__file__).split('.')[0]
args = CovJanArgs(test_name, is_create_output_folder=False)
args.alpha = 0.8
n_randomizations = 1008
...
for i in tqdm(range(n_randomizations), desc='Propagating random scores', total=n_randomizations):
    ...
    bootstrapped_values = np.random.choice(values, len(propagation_input))
    shuffled_input = dict(zip(propagation_input.keys(), bootstrapped_values))
