...
for i in range(n_tests):
    ...
    for c, condition in enumerate(prior_set_conditions):
        ...
        log2fc_scores_by_pathway = [[log2fc_scores[id] for id in gene_set if id in log2fc_scores] for gene_set
                                    in genes_by_pathway_filtered]

        fc_p_vals, fc_dir, fc_z_score, prop_p_vals, prop_dir, prop_z_score = [], [], [], [], [], []
        for g, gene_set in enumerate(genes_by_pathway_filtered):
            random_genes = [id for id in log2fc_scores if id not in gene_set]

            fc_res = statistic_test(log2fc_scores_by_pathway[g], [log2fc_scores[id] for id in random_genes])
