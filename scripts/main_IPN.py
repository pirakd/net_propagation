...
true_score = propagate([id for id in propagation_input.keys()], propagation_input, matrix, gene_indexes, num_genes, args)
...
for i in tqdm(range(n_randomizations), desc='Propagating random scores', total=n_randomizations):
    ...
    random_scores.append(propagate([id for id in shuffled_input.keys()], shuffled_input, matrix, gene_indexes, num_genes
                                   , args))
